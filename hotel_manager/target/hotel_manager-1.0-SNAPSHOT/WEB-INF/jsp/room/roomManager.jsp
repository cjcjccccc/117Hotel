...
<script>
    layui.use(['form', 'table', 'laydate', 'jquery', 'layer','upload','layedit'], function () {
        ...
        //查询楼层列表
        $.get("${pageContext.request.contextPath}/admin/floor/floorList",function(result){
            var html = "";
            //循环遍历数据
        ...
        //查询房型列表
        $.get("${pageContext.request.contextPath}/admin/roomType/roomTypeList",function(result){
            var html = "";
            //循环遍历数据
            for (let i = 0; i <result.length; i++) {
        ...
        upload.render({
            elem:".thumbBox",//绑定元素
            url:"${pageContext.request.contextPath}/admin/room/uploadFile",//提交地址
        ...
        function openAddWindow() {
            ...
            detailIndex = layedit.build("roomDesc",{
                uploadImage:{
                    url:"${pageContext.request.contextPath}/admin/room/detailUpload",//提交地址
                    type:"post"//提交方式
                }
        ...
        function openUpdateWindow(data) {
            mainIndex = layer.open({
                ...
                success:function () {
                    //提交地址
                    url = "${pageContext.request.contextPath}/admin/room/updateRoom";
                    //数据回显
        ...
        function deleteById(data) {
            ...
            if(data.status!=1){ //1为可预订
                ...
                layer.confirm("确定要删除[<font color=red>"+data.title+"("+data.roomNum+")"+"</font>]房间吗",{icon:3,title:"提示"},function (index) {
                    //发送删除请求
                    $.post("${pageContext.request.contextPath}/admin/room/deleteById",{"id":data.id},function(result){
                        if(result.success){
                            layer.alert(result.message,{icon:1});
                            tableIns.reload();
                        }else{
