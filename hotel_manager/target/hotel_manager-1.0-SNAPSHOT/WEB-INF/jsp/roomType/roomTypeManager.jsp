...
<script>
    layui.use(['form', 'table', 'laydate', 'jquery', 'layer','upload'], function () {
        ...
        upload.render({
            elem:".thumbBox",//绑定元素
            url:"${pageContext.request.contextPath}/admin/roomType/uploadFile",//提交地址
        ...
        function openUpdateWindow(data) {
            mainIndex = layer.open({
                ...
                success:function () {
                    //提交地址
                    url = "${pageContext.request.contextPath}/admin/roomType/updateRoomType";
                    //数据回显
        ...
        function deleteById(data) {
            //查询该房型是否存在房间信息
            $.get("${pageContext.request.contextPath}/admin/roomType/checkRoomTypeHasRoom",{"roomTypeId":data.id},function(result){
                if(result.exist){
                    layer.alert(result.message,{icon:2});
                }else{
                    //提示用户是否删除该房型
                    layer.confirm("确定要删除[<font color='red'>"+data.typeName+"</font>]房型吗?",{icon:3,title:"提示"},function (index) {
                        //发送删除请求
                        $.post("${pageContext.request.contextPath}/admin/roomType/deleteById",{"id":data.id},function(result){
                            if(result.success){
                                layer.alert(result.message,{icon:1});
                                //刷新数据表格
                                tableIns.reload();
