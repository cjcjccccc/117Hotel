...
<script>
    layui.use(['form', 'table', 'laydate', 'jquery', 'layer'], function () {
            ...
            cols: [[
                ...
                {field: 'status', width: 80, title: '状态', align: "center",templet:function (d) {
                        if(d.status==1){
                            ...
                            return "<font color='green'>已退房</font>";
                        }
                    }},
        ...
        function confirmOrders(data) {
            ...
            if(data.status==1){
                //发送请求
                $.post("${pageContext.request.contextPath}/admin/orders/confirmOrders",{"id":data.id},function(result){
                    if(result.success){
        ...
        function batchConfirm() {
            ...
            if(checkStatus.data.length>0){
                ...
                layer.confirm("确定要确认这些订单吗?",{icon:3,title:"提示"},function (index) {
                    ...
                    var ids = idArr.join(",");
                    //发送请求
                    $.post("${pageContext.request.contextPath}/admin/orders/batchConfirm",{"ids":ids},function(result){
                        if(result.success){
                            layer.alert(result.message,{icon:1});
                            //刷新数据表格
                            tableIns.reload();
